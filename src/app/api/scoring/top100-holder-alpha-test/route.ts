...
export async function GET(request: Request) {
  ...
  try {
    ...
    const holderAlpha = computeTop100HolderAlphaV1(walletAlphaBatch);
    const sortedWallets = [...walletAlphaBatch].sort(
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        averageWalletAlphaV2: average(
          walletAlphaBatch.map((wallet) => wallet.scores.walletAlphaV2)
        ),
        ...
        walletsReachedEndOfHistory:
          providerResult.data.walletsReachedEndOfHistory,
        walletsStoppedByPageLimit:
          providerResult.data.walletsStoppedByPageLimit,
    ...
    return errorResponse(
      error instanceof Error
        ? error.message
        : "Top100 Holder Alpha test request failed.",
      500
    );
  }
}
