...
const walletPersonalitiesCache = new Map<
  string,
  {
    expiresAt: number;
    body: WalletPersonalitiesResponse;
  }
>();
...
export async function GET(request: Request) {
  ...
  const cacheKey = `${mappedChain}:${tokenAddress.toLowerCase()}:${limit}`;
  const cached = walletPersonalitiesCache.get(cacheKey);
  ...
  try {
    ...
    const body: WalletPersonalitiesResponse = {
      tokenAddress,
      chain: mappedChain,
      limit,
      personalities,
    ...
    walletPersonalitiesCache.set(cacheKey, {
