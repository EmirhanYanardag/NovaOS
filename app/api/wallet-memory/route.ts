...
const walletMemoryCache = new Map<
  string,
  {
    expiresAt: number;
    body: WalletMemoryResponse;
  }
>();
...
export async function GET(request: Request) {
  ...
  const cached = walletMemoryCache.get(cacheKey);
  ...
  try {
    ...
    const body: WalletMemoryResponse = {
      walletAddress,
      chain: mappedChain,
    ...
    walletMemoryCache.set(cacheKey, {
