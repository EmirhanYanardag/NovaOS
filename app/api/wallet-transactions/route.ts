...
export async function GET(request: Request) {
  ...
  const cacheKey = `${mappedChain}:${normalizedWallet}:${normalizedToken || "all"}`;
  ...
  try {
    ...
    const transactions = transfers
      .map((transfer) => normalizeTransfer({ transfer, walletAddress }))
      .filter((transaction) =>
        normalizedToken
    ...
    const warnings = buildWarnings({ transactions, tokenAddress });
    const body: WalletTransactionsResponse = {
      walletAddress,
      chain: mappedChain,
